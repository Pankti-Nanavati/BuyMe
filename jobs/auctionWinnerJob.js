...
const runAuctionWinnerJob = async () => {
    try {
        ...
        const endedAuctionsQuery = 'Select A.auction_id, A.minimum_price, A.email_id, A.product_id P.product_name from `bm_auction_system`.`auction` A inner join `bm_auction_system`.`product` P on A.product_id = P.product_id where end_time < NOW() and has_winner = 0;';
