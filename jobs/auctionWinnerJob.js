...
const runAuctionWinnerJob = async () => {
    try {
        ...
        for(let i = 0; i < endedAuctions.length; i++){
            const highestBidQuery = 'Select MAX(amount), email_id, bidding_timestamp from `bm_auction_system`.`bid` where auction_id = ?;';
            const [highestBid] = await db.execute(highestBidQuery, [endedAuctions[i].auction_id]);
            if (endedAuctions[i].minimum_price <= highestBid[0].amount){
            ...
            const updateAuction = 'Update table `bm_auction_system`.`auction` set has_winner = 1 where auction_id = ?;';
            const [updateAuctionRes] = db.execute(updateAuction, [endedAuctions[i].auction_id]);
